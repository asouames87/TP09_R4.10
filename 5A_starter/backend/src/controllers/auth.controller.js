...
export const updateFavoriteGenres = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(favoriteGenres)) {
      return res.status(400).json({
        success: false,
        message: "favoriteGenres doit être un tableau de chaînes de caractères",
      });
    }
...
// @desc    Obtenir les films likés par l'utilisateur
// @route   GET /api/auth/me/likes
// @access  Private
export const getLikedMovies = async (req, res, next) => {
  try {
    const movies = await Movie.getLikedByUser(req.user._id);
