...
function MovieCard({ movie }) {
  ...
  const { isAuthenticated, user, likedMovies, setLikedMovies } = useAuth();
  ...
  const handleLike = async (e) => {
    ...
    if (!isAuthenticated()) {
      warning("Veuillez vous connecter pour liker un film");
      return;
    }
    ...
    try {
      const response = isLiked
        ...
        : await moviesAPI.like(id);
      if (response.success) {
        const updatedMovie = response.data;
        // Mettre à jour les films likés dans le contexte
        const updatedLikedMovies = isLiked
          ? likedMovies.filter((likedMovie) => likedMovie._id !== id)
          : [...likedMovies, updatedMovie];
        setLikedMovies(updatedLikedMovies);
        setLikes(updatedMovie.likes.length);
      }

      setIsLiked(!isLiked);
    } catch (err) {
      error(err.message || "Erreur lors du like/unlike du film:");
    }
  };
